...
export function ERPilotPage(){
 ...
 const ask=async(e:FormEvent)=>{e.preventDefault();if(!question.trim())return;setBusy(true);try{setAnswer(await api.erpilot.chat({message:question.trim(),conversation:[]} ) as Row);}catch(err){toast.error(err instanceof Error?err.message:"ERPilot is unavailable");}finally{setBusy(false)}};
...
function ManufacturingPlantsPage(){
  ...
  const payload=()=>({name:String(form.name||"").trim(),code:String(form.code||"MFG").trim().toUpperCase(),address:String(form.address||"").trim(),city:String(form.city||"").trim(),lat:Number(form.lat),lng:Number(form.lng),capacity:Number(form.capacity||0),status:String(form.status||"Active"),notes:String(form.notes||"")});
