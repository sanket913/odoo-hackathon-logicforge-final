...
export function Toolbar({ search, setSearch, children, onNew, newLabel }: { search: string; setSearch: (v: string) => void; children?: ReactNode; onNew?: () => void; newLabel?: string }) {
  return <div className="mb-4 flex flex-nowrap items-center gap-3 overflow-x-auto rounded-lg border border-border bg-card p-3"><div className="relative min-w-72 flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search live records..." className="h-10 w-full rounded-md border border-input bg-background pl-9 pr-3 text-sm focus:ring-2 focus:ring-ring" /></div>{children}{onNew && <Button className="shrink-0" onClick={onNew}><Plus />{newLabel || "New Record"}</Button>}</div>;
}
